...
# PIPELINE_GOOGLE_API_KEY takes priority (avoids conflict with ADK's Vertex AI session service)
api_key = os.getenv("PIPELINE_GOOGLE_API_KEY", "") or get_config_val("api_keys", "google_api_key", "GOOGLE_API_KEY", "")
gcp_project = os.getenv("GOOGLE_CLOUD_PROJECT", "")
gcp_location = os.getenv("GOOGLE_CLOUD_LOCATION", "us-central1")

if api_key:
    # API key mode — force consumer Gemini API (not Vertex AI)
    gemini_client = genai.Client(api_key=api_key, vertexai=False)
    print("Initialized Gemini Client with API Key")
elif gcp_project:
    # Vertex AI mode — service account auth (Agent Engine without API key)
    gemini_client = genai.Client(vertexai=True, project=gcp_project, location=gcp_location)
    print(f"Initialized Gemini Client with Vertex AI ({gcp_project}/{gcp_location})")
else:
...
if anthropic_api_key and AsyncAnthropic is not None:
    ...
    if not AsyncAnthropic:
        print("Warning: anthropic package not installed. Skipping Anthropic client.")
    else:
...
if openai_api_key and AsyncOpenAI is not None:
    ...
    if not AsyncOpenAI:
        print("Warning: openai package not installed. Skipping OpenAI client.")
    else:
